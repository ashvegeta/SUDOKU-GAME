...
class SudokuGrid {
  grid: number[][] = [];
  boxHashTable = new Map<number, number[]>();
  ...
  // create custom grid
  createCustomGrid(customGrid: number[][]) {
  ...
  checkPosFilled(row: number, col: number): boolean {
  ...
  checkRow(row: number, val: number): boolean {
  ...
  checkCol(col: number, val: number): boolean {
  ...
  checkBox(row: number, col: number, val: number): boolean {
...
function sendMove(event: Event): boolean {
  ...
  const valInput = document.getElementById("valInput") as HTMLInputElement;
  ...
  if (grid.checkPosFilled(row, col)) return false;
  if (grid.checkRow(row, val)) return false;
  if (grid.checkCol(col, val)) return false;
  if (grid.checkBox(row, col, val)) return false;

  // Update the HTML table
  grid.grid[row][col] = val;
  const table = document.getElementById("sudokuGrid") as HTMLTableElement;
